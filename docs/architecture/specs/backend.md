...
**Примеры типов.** `Building`, `Apartment` (агрегаты/сущности по выбранной модели).
...
**Примеры типов.** `BuildingsController`, фильтры, привязка моделей к командам/запросам MediatR.
